#!/bin/bash
#
# This script will deploy the JIRA manifest on a specified
...
print_usage()
{
    ...
    echo " -p <mysql-provider>   is the FQN of the mysql provider that will hold the JIRA config"
    echo " -t <jira-version-tag> is the version tag of the JIRA Docker image"
...
apc manifest deploy jira-manifest.json -- \
